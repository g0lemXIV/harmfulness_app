...
def test_spacy_tokenizer():
    # test case 1
    case = "@anonymized_account @anonymized_account Zapewne ci," \
           " co grali przez większość rundy w CLJ i nie ponieśli ani jednej porażki."
    ...
    # test case 2
    case = "@anonymized_account @account @anonymized_account" \
           " Dlatego zaznaczyłem, że jest to wątpliwość." \
           " Nie wiem, czy w wieku 18 lat się jeszcze rośnie."
    pre_out = _clean_text(case)
    output = "zaznaczyłem wątpliwość wiedzieć wieko rosnąć"
    text_case = _spacy_tokenizer(CORPUS["pl"](pre_out))
    assert text_case == output
    # test case 3
    case = "@anonymized_account Żal ci biedaku??? Gdyby nie Kaczyński" \
           " to by je twoi przyjaciele z PO rozkradl"
    pre_out = _clean_text(case)
